...
// === НАСТРОЙКИ ===
const TG_BOT = 'phuket_nestandart_bot';
...
// URL path takes priority over localStorage (nestandart.online/phuket, /pattaya, etc.)
const _urlMarket = window.location.pathname.split('/').filter(Boolean)[0];
let currentCity = (CITIES[_urlMarket] ? _urlMarket : null) || localStorage.getItem('nestandart_city') || null;
...
tCards.forEach(card => {
  card.addEventListener('click', (e) => {
    ...
    if (tourId) {
      window.location.href = `tours/${tourId}.html`;
    }
  });
});
