...
> Последнее обновление: 2026-06-22 · v4 — КПТ зафиксирован как главная метрика
...
Это единственная метрика, которая имеет значение в «Нестандартном Отдыхе».
...
### Активные workflows (6 штук)

| Workflow | Триггер | Действие |
|----------|---------|----------|
| **КотЭ — AI Агент с памятью** (`doCUKEZQpLQjDmxP`) | Telegram message | 21 нода: извлечь → upsert клиент → контекст → собрать промпт → **backend `/api/v1/ai/chat`** (каскад) → ответ → сохранить; при готовой брони → **запись в CRM (`app_upsert_lead`) + уведомление менеджеру + создание оплаты YooKassa + ссылка клиенту** |
...
**Характер:** КотЭ ведёт себя как КОТ — мурчит/мяукает в меру, кошачьи образы и реакции, но коротко и по делу (1 кошачий штрих на сообщение, не списком). Усилено финальной директивой в конце системного промпта (recency).
...
### 🔔 Протокол брони (с 2026-06-21)

КотЭ **сам собирает все данные** перед обращением к менеджеру — менеджеру уходит готовый структурированный пакет, а не «сырое» сообщение.

- Поля, которые КотЭ собирает в диалоге: **имя, телефон, тур, дата, отель, номер комнаты, всего человек → из них взрослых / детей / младенцев до 4 лет** (младенцы до 4 — бесплатно).
- Когда все поля собраны, КотЭ добавляет в конец ответа технический блок `[[BRON]]…[[/BRON]]` (по строке `поле: значение`).
- Узел **`💬 Достать ответ`** парсит блок → `booking`/`booking_ready`, **вырезает** блок из видимого клиенту текста.
- **`🔔 Бронь?`** срабатывает по `booking_ready === true` (а НЕ по ключевым словам) — только когда данные полные.
- **`📥 Бронь в CRM`** (параллельно уведомлению) — пишет бронь в Supabase через `app_upsert_lead` (anon-доступ, дедуп клиента по `tg_chat_id`, бронь по уникальному `external_id=tg_<chat>_<ts>`). Разбивка взрослые/дети/младенцы и дата (текстом) кладутся в `p_comment`; `p_people`=всего; `p_date_start`=null (date-тип не принимает «15 января»).
- **`📲 Уведомить о брони`** шлёт менеджеру: имя, телефон, тур, дата, отель+комната, всего/взрослые/дети/младенцы, chat_id.
- Инструкция КотЭ живёт в `SYSTEM_PROMPT` узла `✍️ Собрать промпт` (зеркало — `platform/kote/prompt.txt`).
- ⚠️ Писать в `clients`/`bookings` ТОЛЬКО через `app_upsert_lead` (прямые INSERT запрещены). adults/children отдельными колонками функция не заполняет — поэтому разбивка идёт в comment.
