...
|----------|---------|----------|
| **КотЭ — AI Агент с памятью** (`doCUKEZQpLQjDmxP`) | Telegram message | 18 нод: извлечь → upsert клиент → контекст → собрать промпт → **backend `/api/v1/ai/chat`** (каскад) → ответ → сохранить; при готовой брони → **запись в CRM (`app_upsert_lead`) + уведомление менеджеру** |
